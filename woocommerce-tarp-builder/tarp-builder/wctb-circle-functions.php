...
// Assign Edit Boxes to Circle Tarp Builder's Product Edit Page.
add_action( 'add_meta_boxes', 'custom_circle_tarpbuilder_postboxes' );
function custom_circle_tarpbuilder_postboxes() {
    global $post;
    if ( !is_object( $post ) ) {
        return;
    }
    ...
    if ('circletarpbuilder' === $product->get_type()) {
        add_meta_box(
            'postbox_for_circle_tarpbuilders',
            'Circle Tarp Builder Options',
            'render_postbox_inputs_for_circle_tarpbuilders',
            'product',
            'advanced',
            'high'
        );
...
function render_postbox_inputs_for_circle_tarpbuilders( $post ) {
    require_once('postbox-inputs-circle.php'); // Outputs the inputs on the product edit page which in turn defines what shows up on the front end.
}

// Save the inputs from the Edit Boxes on the Product Edit Page.
add_action('save_post', 'save_postbox_inputs_for_circle_tarpbuilders');
function save_postbox_inputs_for_circle_tarpbuilders($post_id) {

    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) { return; }
    if (!current_user_can('edit_post', $post_id)) { return; }
    if ('product' !== get_post_type($post_id)) { return; }
    if (!wc_get_product($post_id) || 'circletarpbuilder' !== wc_get_product($post_id)->get_type()) { return; }
...
add_filter( 'woocommerce_add_cart_item_data', 'add_wctb_circle_cart_item_data', 10, 3 );
function add_wctb_circle_cart_item_data( $cart_item_data, $product_id, $variation_id ) {
    $product = wc_get_product( $product_id );

	if( $product->is_type( 'circletarpbuilder' ) ) {
        ...
        $circumscribed_square_footage = filter_input(INPUT_POST, "wctb-fe-circumscribed-square-footage", FILTER_VALIDATE_FLOAT);
        ...
        $grommetspacing = sanitize_text_field($_POST["wctb-fe-grommet-spacing"]);

        $customgrommetsidepricing = 0;
        // $grommetspacing_inches = sanitize_text_field($_POST["wctb-fe-grommet-spacing-diameter-inches"]);

        // // A method for adding custom grommet side pricing based on the grommet spacing.
        // if ($grommetspacing_inches > 24) {
        //     $customgrommetsidepricing += 1.25;
        // } else if ($grommetspacing_inches > 18) {
        //     $customgrommetsidepricing += 1.49;
        // } elseif ($grommetspacing_inches > 12) {
        //     $customgrommetsidepricing += 1.73;
        // } elseif ($grommetspacing_inches > 6) {
        //     $customgrommetsidepricing += 1.87;
        // } else if ($grommetspacing_inches > 0) {
        //     $customgrommetsidepricing += 2.50;
        // }
        ...
        if ($chosen_edge_num == 99) {
            ...
            $cart_item_data['grommetspacing'] = $grommetspacing;
        ...
        if ($edge_option_price > 0 || $chosen_edge_num > 1) {
            $dynamicPrice = $circumscribed_square_footage * ($material_baseprice + $material_hemmedprice + $edge_option_price);
        } else {
            $dynamicPrice = $circumscribed_square_footage * $material_baseprice;
        }
...
add_action( 'woocommerce_before_calculate_totals', 'wctb_adjust_circle_cart_item_price', 10, 1 );
function wctb_adjust_circle_cart_item_price( $cart_obj ) {
	...
	foreach( $cart_obj->get_cart() as $key=>$value ) {
		...
		if( $product_type == 'circletarpbuilder' && isset( $value['dynamicPrice'] ) ) {
			$price = $value['dynamicPrice'];
			$value['data']->set_price( ( $price ) );
		}

        if( $product_type == 'circletarpbuilder' && isset( $value['dynamic_weight'] ) ) {
...
add_filter( 'woocommerce_cart_item_subtotal', 'wctb_circle_cart_item_subtotal', 10, 3 );
function wctb_circle_cart_item_subtotal( $subtotal, $cart_item, $cart_item_key ) {
    $product_type = $cart_item['data']->get_type();

    if( $product_type == 'circletarpbuilder' && isset( $cart_item['dynamicPrice'] ) ) {
...
add_filter( 'woocommerce_get_item_data', 'display_wctb_circle_item_details_cart_checkout', 10, 2 );
function display_wctb_circle_item_details_cart_checkout( $item_data, $cart_item ) {
    $product_type = $cart_item['data']->get_type();

    if( $product_type != 'circletarpbuilder' ) {
        return $item_data;
    }
    ...
    $grommetspacing = isset( $cart_item['grommetspacing'] ) ? $cart_item['grommetspacing'] : '';
...
add_action( 'woocommerce_add_order_item_meta', 'display_wctb_circle_item_details_order_thankyou_emails', 10, 3 );
function display_wctb_circle_item_details_order_thankyou_emails( $item_id, $values, $cart_item_key ) {
    ...
    if( $product_type == 'circletarpbuilder' ) {
        ...
        $grommetspacing = isset( $values['grommetspacing'] ) ? $values['grommetspacing'] : '';
        ...
        if ( ! empty( $grommetspacing ) ) {
            wc_add_order_item_meta( $item_id, 'Grommet Spacing', $grommetspacing );
        }
    }
